...
> [!Important]
> 本スクリプトではAPIキーを直打ちしていますが、実環境ではSecret Manager等で管理してください。
> なお、APIキーの共有範囲は最小限にとどめてください。権限過多のキーが漏洩するとセキュリティリスクになります。
> 多数の部署でコンテナイメージを利用する場合は、ECR / GAR / ACR 等に一旦Pushして共有する方法を推奨します。
